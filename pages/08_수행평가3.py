...
# 3. 데이터 로드 및 파생변수 생성 (의미 도출을 위한 가공)
@st.cache_data
def load_data():
    ...
    # 분모가 0이 되는 것을 방지하기 위해 1 이상인 경우만 계산
...
try:
    ...
    st.error("CSV 파일을 찾을 수 없습니다. GitHub 저장소에 파일이 함께 있는지 확인해 주세요.")
    st.stop()
...
with col3:
    st.metric("전체 평균 항공기당 승무원 수", f"{avg_crew_per_plane} 명", help="전체 입항 승무원을 전체 입항 항공기 수로 나눈 평균값입니다.")

st.markdown("---")

# 탭 구조를 활용한 심층 분석 세션 분리
...
with tab1:
    ...
    fig1.update_layout(plot_bgcolor='white', paper_bgcolor='white', hovermode="x unified", font=dict(color='#4A3525'))
    fig1.update_yaxes(showgrid=True, gridcolor='#E6D7C3')
    st.plotly_chart(fig1, use_container_width=True)

# --- TAB 2: 항공기당 승무원 밀도 분석 ---
with tab2:
    st.subheader("✈️ 공항별 항공기 1대당 평균 승무원 수 분석")
    st.markdown("""
        **이 차트가 가지는 의미:** 항공기 1대당 탑승한 승무원 수가 많을수록 해당 공항에는 주로 **대형 항공기(광동체기)**가 취항하거나 장거리 노선 위주로 편성되어 있음을 시사합니다.
    """)
    ...
    fig2.update_layout(plot_bgcolor='white', paper_bgcolor='white', hovermode="x unified", font=dict(color='#4A3525'))
...
with tab3:
    ...
    st.markdown("""
        **이 차트가 가지는 의미:** - **(+) 양수 값:** 들어온 것보다 **나간 것(출항)**이 많음 (해당 공항을 기점으로 출발하는 정비 회항이나 승무원 교대 스케줄 분포 영향)
        - **(-) 음수 값:** 들어온 것보다 **덜 나감** (공항 내 체류 시간 증가 또는 데이터 집계 시점의 잔류 상태 반영)
    """)
    ...
    fig3.update_layout(plot_bgcolor='white', paper_bgcolor='white', font=dict(color='#4A3525'))
    fig3.update_yaxes(showgrid=True, gridcolor='#E6D7C3')
    st.plotly_chart(fig3, use_container_width=True)

# 8. 데이터 원본 확인
st.markdown("---")
with st.expander("📄 데이터셋 원본 및 계산된 인덱스 확인하기"):
